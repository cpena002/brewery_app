class BreweriesController < ApplicationController
  ...
  def get_events
      ...
      @tastings.each do |tasting|
        events << { id: tasting.id, title: tasting.name, start: tasting.start,
        end: tasting.end,
        url: Rails.application.routes.url_helpers.event_path(tasting.id),
        textColor: 'black',
        color: 'yellow'
        }
      end
  ...
  def map_location
    ...
    marker.infowindow("<em>" + brewery.address + "</em>")
  end
  ...
  private
    ...
    def brewery_params
      params.require(:brewery).permit(:name, :description, :address, :latitude, :longitude)
    end
end
